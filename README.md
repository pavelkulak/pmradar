...
7. Проверьте `https://pm.example.com/health` — успешный ответ: `{"status":"ok"}`. Войдите по адресу `https://pm.example.com`, откройте «Интеграции» и нажмите «Проверить подключения».
8. Добавьте ботов в тестовые рабочие чаты, отправьте новое сообщение и убедитесь, что карточка появилась в разделе «Требуют внимания». Откройте чат в мессенджере и нажмите «Ознакомился»; следующее сообщение должно снова пометить чат как непросмотренный.
...
Страница «Интеграции» делает реальные серверные запросы к `getWebhookInfo` Telegram и `GET /subscriptions` MAX. MAX не предоставляет счётчик отложенных webhook-событий; это поле оставлено пустым. Страница показывает фактическую подписку, последнее время получения события и время последней проверки.
